...
\section{Motivation}
As explained by Nicholas Petrick~\cite{50}, "it has long been recognized that clinicians do not always make optimal use of the data acquired by an imaging device. The limitations of the human eye-brain system, limitations in training and experience, and factors such as fatigue, distraction, and satisfaction of search may all contribute to suboptimal use of available information". In these circumstances, CAD systems can contribute to address some of these issues by being used as an aid for clinicians. In fact, they provide multiple advantages.\\
First of all, used as an extra diagnostic, they decrease the probability to miss positive diagnoses. Then, they can also speed up the diagnosis by proposing regions of interest to clinicians, which in time, will reduce the screening costs. Similarly, lower screening costs mean a greater accessibility of screening tests, which results in an earlier cancer detection. Another benefit of using CAD systems, specially the one based on Deep Learning, is the number of cases that these latter have "seen". Indeed, in order to create efficient Deep Learning models, big amounts of data are needed. Consequently, efficient CAD systems have seen a lot of cancer examples whereas beginner clinicians have probably seen less a smallest number of them.\\
However, all these advantages are still nowadays partially theoretical and the research is still going on to improve existent CAD systems. Problems like poor detection rates, small amounts of available data and bad levels of generalization are still recurrent. 
