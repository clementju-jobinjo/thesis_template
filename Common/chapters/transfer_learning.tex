...
However, in order for the transfer learning to be useful, common low level features have to be present among the datasets on which the model is trained. Consequently, the goal of this experiment is to use transfer learning on three datasets of three different body parts, that are prostate, brain and lung, in order to check if they share common features that could be useful to improve the performance of the model on a specific dataset. In this experiment, the model trained to classify prostate lesions is then trained to classify brain tumors and finally lung lesions. At the end of the process, the model is once again trained on the prostate dataset and the performances of the model at the beginning and at the end of the experiment are compared.
...
All training graphs and the metrics they achieved on the validation sets are shown on figures XXXX.\\
Table \ref{table:best_model_performance} summaries the performance of the best model on the validation and test sets at each step of the experiment.
...
\label{table:best_model_performance}
\end{table}
\section{Discussion}
...
2) Features of brain and lung images are useful to classify prostate lesions\\
	--> Explain that the frozen curve immediately start to increase, and then get stable. Furthermore they allow us to reach good performance metrics, describe them.\\
	--> Explain the global performance graph in details\\\\
