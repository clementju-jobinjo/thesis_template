...
The whole transfer learning is composed of multiple steps. In order to maximize the performance during the whole process, the hyperparameters were manually tuned at each step, following a grid search approach to find the best ones. Figure \ref{table:tl_hyperparameters_at_each_step} presents the best hyperparameters used at each time of the experiment. Note that, as it was the case in the research paper reproduction, the first model was also initialized picking a the best model among 200 initializations. The model was implemented with PyTorch using the Adam optimizer to update its weight and the complete succession of the whole process took approximately 50 hours on an Tesla v100 XSM2 graphic card.
...
\subsection{Training}
This transfer learning experiment was performed manually. This means that each step was run sequentially, each time tuning the hyperparameters (learning rate and dropout probability) to reach the best performance possible.
